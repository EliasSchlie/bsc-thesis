...
For the inferential analysis, qwen3.5-9b is excluded by the pre-registered baseline rule (framing A on rows-with-content gives ~6%, near the lower exclusion bound). The 60% of cells where the model did terminate produce usable descriptive estimates and are reported alongside the loop rate.
